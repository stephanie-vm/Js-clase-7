...
//Esta función contiene el valor de las monedas y las opciones del drpdown para cambiarlo.
function CambiarMoneda(data) {
    const valorMoneda = document.querySelector('select').value;
    if (valorMoneda === 'GBP') {
        const GBP_INFO = data.bpi.GBP;
        priceElement.innerHTML = `${GBP_INFO.code}${GBP_INFO.symbol} ${GBP_INFO.rate}`;
    } else if (valorMoneda === 'EUR') {
